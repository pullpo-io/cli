...
func NewCmdList(f *cmdutil.Factory, runF func(*ListOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		`),
		Example: heredoc.Doc(`
			$ pullpo issue list --label "bug" --label "help wanted"
			$ pullpo issue list --author monalisa
			$ pullpo issue list --assignee "@me"
			$ pullpo issue list --milestone "The big 1.0"
			$ pullpo issue list --search "error no:assignee sort:created-asc"
		`),
		Aliases: []string{"ls"},
