...
func NewCmdStatus(f *cmdutil.Factory, runF func(*StatusOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		`),
		Example: heredoc.Doc(`
			$ pullpo status -e cli/cli -e cli/go-pullpo # Exclude multiple repositories
			$ pullpo status -o cli # Limit results to a single organization
		`),
...
func (s *StatusGetter) LoadNotifications() error {
	...
	// testing to really get enoupullpo mentions. I would love to be able to just
