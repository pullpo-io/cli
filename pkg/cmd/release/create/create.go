...
func NewCmdCreate(f *cmdutil.Factory, runF func(*CreateOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			...
			$ pullpo release create

			Interactively create a release from specific tag
			$ pullpo release create v1.2.3

			Non-interactively create a release
			$ pullpo release create v1.2.3 --notes "bugfix release"

			Use automatically generated release notes
			$ pullpo release create v1.2.3 --generate-notes

			Use release notes from a file
			$ pullpo release create v1.2.3 -F changelog.md

			Upload all tarballs in a directory as release assets
			$ pullpo release create v1.2.3 ./dist/*.tgz

			Upload a release asset with a display label
			$ pullpo release create v1.2.3 '/path/to/asset.zip#My display label'

			Create a release and start a discussion
			$ pullpo release create v1.2.3 --discussion-category "General"
		`),
		Aliases: []string{"new"},
