...
func explainer() string {
	return heredoc.Doc(`
		...
		pullpo uses the default repository for things like:
...
func NewCmdSetDefault(f *cmdutil.Factory, runF func(*SetDefaultOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			...
			$ pullpo repo set-default

			Set a repository explicitly:
			$ pullpo repo set-default owner/repo

			View the current default repository:
			$ pullpo repo set-default --view
...
func setDefaultRun(opts *SetDefaultOptions) error {
	...
	if opts.ViewMode {
		if currentDefaultRepo != nil {
			...
			fmt.Fprintln(opts.IO.Out, "no default repository has been set; use `pullpo repo set-default` to select one")
		}
		return nil
	}
