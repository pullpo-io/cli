...
func NewCmdEdit(f *cmdutil.Factory, runF func(*EditOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		`, "`"),
		Example: heredoc.Doc(`
			$ pullpo pr edit 23 --title "I found a bug" --body "Nothing works"
			$ pullpo pr edit 23 --add-label "bug,help wanted" --remove-label "core"
			$ pullpo pr edit 23 --add-reviewer monalisa,hubot  --remove-reviewer myorg/team-name
			$ pullpo pr edit 23 --add-assignee "@me" --remove-assignee monalisa,hubot
			$ pullpo pr edit 23 --add-project "Roadmap" --remove-project v1,v2
			$ pullpo pr edit 23 --milestone "Version 1"
		`),
