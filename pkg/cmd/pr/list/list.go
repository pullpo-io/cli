...
func NewCmdList(f *cmdutil.Factory, runF func(*ListOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			...
			$ pullpo pr list --author "@me"

			List only PRs with all of the given labels
			$ pullpo pr list --label bug --label "priority 1"

			Filter PRs using search syntax
			$ pullpo pr list --search "status:success review:required"

			Find a PR that introduced a given commit
			$ pullpo pr list --search "<SHA>" --state merged
    	`),
		Aliases: []string{"ls"},
