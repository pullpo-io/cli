...
func NewCmdProject(f *cmdutil.Factory) *cobra.Command {
	var cmd = &cobra.Command{
		...
		Long:  "Work with GitHub Projects. Note that the token you are using must have 'project' scope, which is not set by default. You can verify your token scope by running 'pullpo auth status' and add the project scope by running 'pullpo auth refresh -s project'.",
		Example: heredoc.Doc(`
			$ pullpo project create --owner monalisa --title "Roadmap"
			$ pullpo project view 1 --owner cli --web
			$ pullpo project field-list 1 --owner cli
			$ pullpo project item-list 1 --owner cli
		`),
		GroupID: "core",
	}
