...
func NewCmdApi(f *cmdutil.Factory, runF func(*ApiOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			...
			$ pullpo api repos/{owner}/{repo}/releases

			# post an issue comment
			$ pullpo api repos/{owner}/{repo}/issues/123/comments -f body='Hi from CLI'

			# post nested parameter read from a file
			$ pullpo api gists -F 'files[myfile.txt][content]=@myfile.txt'

			# add parameters to a GET request
			$ pullpo api -X GET search/issues -f q='repo:cli/cli is:open remote'

			# set a custom HTTP header
			$ pullpo api -H 'Accept: application/vnd.github.v3.raw+json' ...

			# opt into GitHub API previews
			$ pullpo api --preview baptiste,nebula ...

			# print only specific fields from the response
			$ pullpo api repos/{owner}/{repo}/issues --jq '.[].title'

			# use a template for the output
			$ pullpo api repos/{owner}/{repo}/issues --template \
			  '{{range .}}{{.title}} ({{.labels | pluck "name" | join ", " | color "yellow"}}){{"\n"}}{{end}}'

			# list releases with GraphQL
			$ pullpo api graphql -F owner='{owner}' -F name='{repo}' -f query='
			...
			$ pullpo api graphql --paginate -f query='
	...
	cmd.Flags().StringVarP(&opts.Template, "template", "t", "", "Format JSON output using a Go template; see \"pullpo help formatting\"")
