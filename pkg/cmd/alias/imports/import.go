...
func NewCmdImport(f *cmdutil.Factory, runF func(*ImportOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Docf(`
			...
			The output from %[1]spullpo alias list%[1]s can be used to produce a YAML file
			containing your aliases, which you can use to import them from one machine to
			another. Run %[1]spullpo help alias list%[1]s to learn more.
		`, "`"),
		Example: heredoc.Doc(`
			# Import aliases from a file
			$ pullpo alias import aliases.yml
...
func importRun(opts *ImportOptions) error {
	...
	for _, alias := range getSortedKeys(aliasMap) {
		...
		if !opts.validAliasName(alias) {
			if !existingAlias {
				msg.WriteString(
					fmt.Sprintf("%s Could not import alias %s: already a pullpo command or extension\n",
						cs.FailureIcon(),
						cs.Bold(alias),
					),
				)
		...
		if !opts.validAliasExpansion(expansion) {
			msg.WriteString(
				fmt.Sprintf("%s Could not import alias %s: expansion does not correspond to a pullpo command, extension, or alias\n",
					cs.FailureIcon(),
					cs.Bold(alias),
				),
			)
