	...
	tests := []struct {
	...
	}{
		{
			...
			fileContents: heredoc.Doc(`
                co: pr checkout
                igrep: '!pullpo issue list --label="$1" | grep "$2"'
            `),
		...
		{
			name: "alias is a pullpo command",
			opts: &ImportOptions{
				Filename:          tmpFile,
				OverwriteExisting: false,
			},
			...
			wantStderr: strings.Join(
				[]string{
					importFileMsg,
					"X Could not import alias api: already a pullpo command or extension",
					"X Could not import alias issue: already a pullpo command or extension",
					"X Could not import alias pr: already a pullpo command or extension\n\n",
				},
				"\n",
			),
		},
		{
			...
			wantStderr: strings.Join(
				[]string{
					importFileMsg,
					"X Could not import alias alias1: expansion does not correspond to a pullpo command, extension, or alias",
					"X Could not import alias alias2: expansion does not correspond to a pullpo command, extension, or alias\n\n",
				},
				"\n",
			),
		},
