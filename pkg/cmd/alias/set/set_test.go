...
func TestSetRun(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			opts: &SetOptions{
				Name:      "igrep",
				Expansion: "!pullpo issue list | grep",
			},
			wantExpansion: "!pullpo issue list | grep",
			wantStderr:    "- Creating alias for igrep: !pullpo issue list | grep\n✓ Added alias igrep\n",
		},
		{
		...
		},
		{
			...
			wantErrMsg: "X Could not create alias foo: expansion does not correspond to a pullpo command, extension, or alias",
