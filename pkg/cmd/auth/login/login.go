...
func NewCmdLogin(f *cmdutil.Factory, runF func(*LoginOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Docf(`
			...
			If a credential store is not found or there is an issue using it pullpo will fallback to writing the token to a plain text file.
			See %[1]spullpo auth status%[1]s for its stored location.
			...
			Alternatively, pullpo will use the authentication token found in environment variables.
			This method is most suitable for "headless" use of pullpo such as in automation. See
			%[1]spullpo help environment%[1]s for more info.

			To use pullpo in GitHub Actions, add %[1]sGH_TOKEN: ${{ github.token }}%[1]s to %[1]senv%[1]s.
		`, "`"),
		Example: heredoc.Doc(`
			...
			$ pullpo auth login --with-token < mytoken.txt

			# authenticate with a specific GitHub instance
			$ pullpo auth login --hostname enterprise.internal
		`),
...
func loginRun(opts *LoginOptions) error {
	...
	// The go-pullpo Config object currently does not support case-insensitive lookups for host names,
