...
var HelpTopics = []helpTopic{
	{
		name:  "mintty",
		short: "Information about using pullpo with MinTTY",
		long: heredoc.Docf(`
			MinTTY is the terminal emulator that comes by default with Git
			for Windows. It has known issues with gh's ability to prompt a
			user for input.

			There are a few workarounds to make pullpo work with MinTTY:
			...
			- Prefix invocations of pullpo with %[1]swinpty%[1]s, eg: %[1]swinpty pullpo auth login%[1]s.
	...
	},
	{
		...
		long: heredoc.Docf(`
			...
			%[1]sGH_CONFIG_DIR%[1]s: the directory where pullpo will store configuration files. If not specified, 
			...
			%[1]sGH_PATH%[1]s: set the path to the pullpo executable, useful for when pullpo can not properly determine
			its own path such as in the cygwin terminal.
		`, "`"),
	},
	{
		name:  "reference",
		short: "A comprehensive reference of all pullpo commands",
	},
	{
		name:  "formatting",
		...
		example: heredoc.Doc(`
			...
			$ pullpo pr list --json number,title,author
			[
			  {
				"author": {
				  "login": "monalisa"
			...
			$ pullpo pr list --json author --jq '.[].author.login'
			monalisa
			codercat
			cli-maintainer

			# --jq can be used to implement more complex filtering and output changes:
			$ bin/pullpo issue list --json number,title,labels --jq \
			...
			pullpo issue list --json title,url --template '{{range .}}{{hyperlink .url .title}}{{"\n"}}{{end}}'


			# adding the --template flag and modifying the display format
			$ pullpo pr list --json number,title,headRefName,updatedAt --template \
			...
			$ pullpo pr view 3519 --json number,title,body,reviews,assignees --template \
	...
	},
	{
		...
		long: heredoc.Doc(`
			pullpo follows normal conventions regarding exit codes.
...
func helpTopicUsageFunc(w io.Writer, command *cobra.Command) error {
	fmt.Fprintf(w, "Usage: pullpo help %s", command.Use)
	return nil
}
