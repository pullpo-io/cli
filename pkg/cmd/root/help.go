...
func rootHelpFunc(f *cmdutil.Factory, command *cobra.Command, args []string) {
	...
	if longText != "" && command.LocalFlags().Lookup("jq") != nil {
		longText = strings.TrimRight(longText, "\n") +
			"\n\nFor more information about output formatting flags, see `pullpo help formatting`."
	}
	...
	helpEntries = append(helpEntries, helpEntry{"LEARN MORE", heredoc.Docf(`
		Use %[1]spullpo <command> <subcommand> --help%[1]s for more information about a command.
...
func authHelp() string {
	...
	return heredoc.Doc(`
		To get started with GitHub CLI, please run:  pullpo auth login
