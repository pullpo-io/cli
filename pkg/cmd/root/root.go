...
func NewCmdRoot(f *cmdutil.Factory, version, buildDate string) (*cobra.Command, error) {
	...
	cmd := &cobra.Command{
		Use:   "pullpo <command> <subcommand> [flags]",
		Short: "GitHub CLI",
		Long:  `Work seamlessly with GitHub from the command line.`,
		Example: heredoc.Doc(`
			$ pullpo issue create
			$ pullpo repo clone cli/cli
			$ pullpo pr checkout 321
		`),
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			if cmdutil.IsAuthCheckEnabled(cmd) && !cmdutil.CheckAuth(cfg) {
				...
				if parent != nil && parent.Use == "codespace" {
					fmt.Fprintln(io.ErrOut, "To get started with GitHub CLI, please run:  pullpo auth login -s codespace")
				} else {
	...
	if os.Getenv("GH_COBRA") == "" {
		...
		cmd.Flags().Bool("version", false, "Show pullpo version")
