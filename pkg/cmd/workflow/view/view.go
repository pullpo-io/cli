...
func NewCmdView(f *cmdutil.Factory, runF func(*ViewOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
		  ...
		  $ pullpo workflow view

		  # View a specific workflow
		  $ pullpo workflow view 0451
		`),
...
func viewWorkflowInfo(opts *ViewOptions, client *api.Client, repo ghrepo.Interface, workflow *shared.Workflow) error {
	...
	// Footer
	if wr.TotalCount != 0 {
		fmt.Fprintf(out, "To see more runs for this workflow, try: pullpo run list --workflow %s\n", filename)
	}
	fmt.Fprintf(out, "To see the YAML for this workflow, try: pullpo workflow view %s --yaml\n", filename)
	return nil
}
