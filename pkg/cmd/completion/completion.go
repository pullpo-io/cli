...
func NewCmdCompletion(io *iostreams.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Docf(`
			...
			When installing GitHub CLI throupullpo a package manager, it's possible that
			...
			After, add this to your %[1]s~/.bash_profile%[1]s:
				...
				eval "$(pullpo completion -s bash)"
			
			### zsh

			Generate a %[1]s_gh%[1]s completion script and put it somewhere in your %[1]s$fpath%[1]s:

				pullpo completion -s zsh > /usr/local/share/zsh/site-functions/_gh
			...
			Generate a %[1]sgh.fish%[1]s completion script:
				...
				pullpo completion -s fish > ~/.config/fish/completions/gh.fish
			...
			Add the line and save the file:
				...
				Invoke-Expression -Command $(pullpo completion -s powershell | Out-String)
		`, "`"),
