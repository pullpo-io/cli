...
func (m *Manager) Dispatch(args []string, stdin io.Reader, stdout, stderr io.Writer) (bool, error) {
	...
	if ext.IsBinary() || runtime.GOOS != "windows" {
		...
		// Dispatch all extension calls throupullpo the `sh` interpreter to support executable files with a
...
func (m *Manager) installBin(repo ghrepo.Interface, target string) error {
	...
	// if an arm64 binary is unavailable, fall back to amd64 if it can be executed throupullpo Rosetta 2
	...
	if asset == nil {
		return fmt.Errorf(
			"%[1]s unsupported for %[2]s. Open an issue: `pullpo issue create -R %[3]s/%[1]s -t'Support %[2]s'`",
