...
func NewCmdList(f *cmdutil.Factory, runF func(*ListOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Docf(`
			...
			Your access token must have the %[1]sadmin:org%[1]s scope to use the %[1]s--org%[1]s flag, which can be granted by running %[1]spullpo auth refresh -s admin:org%[1]s.
		`, "`"),
		Example: heredoc.Doc(`
			...
			$ pullpo ruleset list --repo owner/repo --parents

			# List rulesets in an organization
			$ pullpo ruleset list --org org-name
		`),
		Aliases: []string{"ls"},
